...
class Gui:
    def __init__(self):
        ...
        self.BLACK = (0, 0, 0)
        self.BACKGROUND = (51, 0, 25)
        self.ORANGE = (236, 134, 34)
    ...
    def show_winner(self, player_color):
        ...
        win_white = pygame.image.load('res/white.png')
        white_pos = win_white.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        win_black = pygame.image.load('res/black.png')
        black_pos = win_black.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        tie = pygame.image.load('res/-TIE.png')
        tie_pos = tie.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        
        if player_color == WHITE:
            msg = pygame.image.load('res/white.png')
        elif player_color == BLACK:
            msg = pygame.image.load('res/black.png')
        else:
            msg = pygame.image.load('res/-TIE.png')
        self.screen.blit(bg, [0,0])
        self.screen.blit(
            msg, msg.get_rect(
    ...
    def get_level(self):
        while True:
            ...
            title = pygame.image.load('res/-COMPUTER-LEVEL.png')
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=80)
            one_txt = pygame.image.load('res/l1.png')
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=180)
            two_txt = pygame.image.load('res/l2.png')
            two_pos = two_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=300)

            three_txt = pygame.image.load('res/l3.png')
    ...
    def showScore(self, blackStr, whiteStr, current_player_color):
        black_background = self.BACKGROUND if current_player_color == WHITE else self.BACKGROUND
        white_background = self.BACKGROUND if current_player_color == BLACK else self.BACKGROUND
